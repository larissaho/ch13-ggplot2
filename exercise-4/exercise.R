...
all.data <- mutate(all.data, pct.voted = (total.voted/registered.voters) * 100)
...
ggplot(data = all.data) + geom_point(mapping = aes(x = total.population, y = pct.voted), color = "blue")

# Create the same scatter plot as above, but put the county names where the dots are
ggplot(data = all.data) + geom_text(mapping = aes(x = total.population, y = pct.voted, label = county), color = "blue")
